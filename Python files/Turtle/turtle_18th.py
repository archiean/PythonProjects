...
def drawPolygon(ttl,x,y,num_side):
    angle = 360/num_side
    ttl.penup()
    ttl.goto(x,y)
    ttl.pendown()
    for iter in range (num_side):
        ttl.forward(200/num_side)
        ttl.left(angle)

def main():
    #draw equilateral triangle
    ttl.color('blue')
    drawPolygon(ttl,-200,0,3)
    #draw square
    ttl.color('red')
    drawPolygon(ttl,-50,0,4)
    #drawPentagon
    ttl.color('forestgreen')
    drawPolygon(ttl,100,0,5)
    #drawOctagon
    ttl.color('DarkOrchid4')
    drawPolygon(ttl,250,0,8)
    #draw a line
    ttl.color('gold4')
